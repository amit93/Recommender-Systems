...
public class TFIDFItemScorer extends AbstractItemScorer {
    ...
    private SparseVector makeUserVector(long user) {
        ...
        for (Rating r: userRatings) {
            ...
            //Unweighted code: In the original implementation, the profile was the sum of
            // the item-tag vectors of all items the user has rated positively (>= 3.5 stars)
        ...
        */
            //weighted user profile: Weighted profile is computed with weighted sum of the item
            // vectors for all items, with weights being based on the user's rating.
            if(p != null)
            {
               sum_rating = sum_rating + p.getValue();
            }

            i++;
        }

        double mean_rating = sum_rating/userRatings.size();
        double weight =0;
        for (Rating r:userRatings)
        {
            Preference p = r.getPreference();
            if(p != null)
            {
               weight = p.getValue() - mean_rating;
                MutableSparseVector mv = model.getItemVector(p.getItemId()).mutableCopy();
                mv.multiply(weight);
                profile.add(mv);
            }
        }
