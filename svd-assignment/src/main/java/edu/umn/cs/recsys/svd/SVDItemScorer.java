...
public class SVDItemScorer extends AbstractItemScorer {
    ...
    public void score(long user, @Nonnull MutableSparseVector scores) {
        ...
        if(userMatrix == null)
        {
            //System.out.println("Null uservector\n");
            scores.clear();
        }
        else{
            for (VectorEntry e: scores.fast(VectorEntry.State.EITHER)) {
                long item = e.getKey();
                // TODO Set the scores
                double baseScore = baselineScorer.score(user,item);
                RealMatrix itemMatrix = model.getItemVector(item);
                RealMatrix featureMatrix = model.getFeatureWeights();

              /*  if(userMatrix == null){
                    //logger.info("User vector is null");
                    System.out.println("FOR: Null uservector\n");
                    continue;
                }*/
                if(featureMatrix == null)logger.info("feature weight matrix is null");

                RealMatrix temp = userMatrix.multiply(featureMatrix);
                RealMatrix scoreMatrix = temp.multiply(itemMatrix.transpose());

                double score = scoreMatrix.getEntry(0,0)+baseScore;
