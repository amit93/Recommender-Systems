...
public class SVDModelBuilder implements Provider<SVDModel> {
    ...
    public SVDModel get() {
        ...
        RealMatrix userFeature = svd.getU();
        userFeature = userFeature.getSubMatrix(0,userFeature.getRowDimension()-1,0,featureCount-1);

        RealMatrix featureWeight = svd.getS();
        featureWeight = featureWeight.getSubMatrix(0,featureCount-1,0,featureCount-1);

        RealMatrix itemFeature = svd.getV();       //Directly fetching the transpose of item feature matrices.
        itemFeature = itemFeature.getSubMatrix(0,itemFeature.getRowDimension()-1,0,featureCount-1);

        SVDModel model = new SVDModel(userMapping,itemMapping,userFeature,itemFeature,featureWeight);
    ...
    private RealMatrix createRatingMatrix(IdIndexMapping userMapping, IdIndexMapping itemMapping) {
        ...
        try {
            for (UserHistory<Event> user: users) {
                ...
                {
                    int v = itemMapping.getIndex(e.getKey());
                    matrix.setEntry(u,v,e.getValue());
                }
